...
func Svg(props a.Props, children ...elem.Node) elem.Node {
	...
	for key, value := range props {
		attrs += fmt.Sprintf(` %s="%v"`, key, value)
	}
...
func LinkElem(href string, element elem.Node) *elem.Element {
	return elem.A(
		a.Props{a.Href: href},
		element,
	)
...
func Bootstrap(title string, props a.Props, children ...elem.Node) *elem.Element {
	content := elem.Html(a.Props{
		...
		elem.Head(nil,
			...
			elem.If(title != "",
				elem.Title(nil, elem.Text(fmt.Sprintf("%s - kradalby.no", title))),
				elem.Title(nil, elem.Text("kradalby.no")),
			),
			elem.Script(a.Props{
...
func PageBase(title string, children ...elem.Node) *elem.Element {
	main := append([]elem.Node{
		elem.Header(nil,
			elem.If(title != "",
				elem.H1(nil, elem.Text(fmt.Sprintf("%s - kradalby.no", title))),
				elem.H1(nil, elem.Text("kradalby.no")),
			),
		),
	}, children...)
