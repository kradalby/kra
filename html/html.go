...
func Alink(href, text string) *Element {
	return A(
		a.Props{a.Href: href},
		Text(text),
	)
...
func LinkElem(href string, elem *Element) *Element {
	return A(
		a.Props{a.Href: href},
		elem,
	)
...
func Bootstrap(title string, props a.Props, children ...Node) *Element {
...
func Base(title string, children ...Node) *Element {
	main := append([]Node{
		Header(nil,
			If(title != "",
				H1(nil, Text(fmt.Sprintf("%s - kradalby.no", title))),
				H1(nil, Text("kradalby.no")),
			),
		),
	}, children...)
	return Bootstrap(title,
		...
		Footer(nil,
			P(nil,
				Text(fmt.Sprintf("Copyright © %d - Kristoffer Dalby", time.Now().Year())),
			),
		),
	)
}
...
func Home() *Element {
	return Base(
		...
		P(nil,
			...
			Text(" and maintainer of "),
			Alink("https://github.com/juanfont/headscale", "Headscale"),
		),
		P(nil,
			Text("Travelling, bouldering, cooking, photography."),
		),
		LinkElem("https://github.com/kradalby", Github(48)),
		LinkElem("https://linkedin.com/in/kradalby", Linkedin(48)),
		LinkElem("https://discordapp.com/users/134699346774982656", Discord(48)),
		LinkElem("https://instagram.com/kradalby", Instagram(48)),
		LinkElem("https://twitter.com/kradalby", Twitter(48)),
	)
}
...
func About() *Element {
	return Base(
		"About me",
		H3(nil, Text("Hi, I am Kristoffer")),
		P(
			nil,
			Text(
		...
		P(nil,
			Text("I work as a Member of Technical Staff at "),
			Alink("https://tailscale.com", "Tailscale"),
			Text(" where I work on both Tailscale and "),
			Alink("https://github.com/juanfont/headscale", "Headscale"),
			Text(", an open source, self-hosted version of the Tailscale control server."),
		),
		P(
			nil,
			Text("I have a Master in Informatics from "),
		...
		Gravatar(400),
		H2(nil, Text("Public speaking")),
		P(
			nil,
			Text("I have done some public speaking, here is a list of publicly available talks:"),
		),
		Ul(nil,
			Li(
				nil,
	...
	return Base(
		...
		Table(nil,
			THead(nil,
				Tr(nil,
					Th(nil, Text("Title")),
					Th(nil, Text("Start date")),
					Th(nil, Text("End date")),
					Th(nil, Text("How I left")),
					Th(nil, Text("Salary")),
					Th(nil, Text("Note")),
				),
			),
		...
		P(nil, Text("I aim to update this when typically change events happens like:")),
		Ul(nil,
			Li(nil, Text("I leave a job")),
			Li(nil, Text("My title or level changes")),
			Li(nil, Text("I get a raise")),
			Li(nil, Text("I am let go")),
		),
		H2(nil, Text("Contributing")),
		P(
			nil,
			Text(
...
func SalaryRows(items data.Salaries) []Node {
	return TransformEach(items, func(sal data.Salary) Node {
		return Tr(nil,
			Td(nil, Text(sal.Title)),
			Td(nil, Text(sal.StartDate)),
			Td(nil, Text(sal.EndDate)),
			Td(nil, Text(sal.HowILeft)),
			Td(nil, Text(sal.Salary)),
			Td(nil, Text(sal.Note)),
		)
	})
}

func Gravatar(size int) *Element {
	url := fmt.Sprintf(
...
func Instagram(size int) *Element {
...
func Linkedin(size int) *Element {
	return NewElement("svg",
		a.Props{
			"height":  fmt.Sprintf("%d", size),
			"width":   fmt.Sprintf("%d", size),
